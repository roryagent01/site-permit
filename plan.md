...
## Phase 0 Foundations
- PL-0.1 (DS-2, DS-6): Supabase auth OTP, workspace/membership tables, RLS baseline
- PL-0.2 (DS-1): App shell routes + navigation + design primitives
- PL-0.3 (DS-10): Server-only env separation + signed upload URLs

## Phase 1 Permits MVP
- PL-1.1 (DS-3.A): Permit templates CRUD
- PL-1.2 (DS-3.B): Permit lifecycle create/edit/status transitions
- PL-1.3 (DS-3.C): Approval chain (single-step then multi-step)
- PL-1.4 (DS-7.A): Realtime permit/approval updates
- PL-1.5 (DS-8.A): Permit PDF export v1

## Phase 2 Contractor Qualifications
- PL-2.1 (DS-4.A): Contractors CRUD + search
- PL-2.2 (DS-4.B): Qualification types CRUD + validity rules
- PL-2.3 (DS-4.C): Contractor qualification records + evidence uploads
- PL-2.4 (DS-4.D, DS-11.A): Reminder settings + digest + logs
